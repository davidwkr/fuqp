...
import com.iodvd.fuqp.ui.util.ThemeUtils.asDrawable
import com.iodvd.fuqp.ui.util.asComponentName
...
object PackageHelper {
    ...
    fun invalidateCache(
        ...
        fuqpApp.globalScope.launch {
            ...
            val cache = withContext(Dispatchers.IO) {
                val pm = fuqpApp.packageManager
                val um = fuqpApp.getSystemService(Context.USER_SERVICE) as UserManager
    ...
    fun loadAppIcon(packageName: String): Drawable = runBlocking {
        getCacheNoThrow()[packageName]?.icon ?:
            android.R.drawable.sym_def_app_icon.asDrawable(fuqpApp)
    }
    ...
    fun loadAppIconFromAppInfo(appInfo: ApplicationInfo): Drawable {
        return if (appInfo.packageName == BuildConfig.APPLICATION_ID) {
            val activityName = findEnabledAppComponent(fuqpApp)
            if (activityName == null) {
                R.mipmap.ic_launcher.asDrawable(fuqpApp)
            } else {
                fuqpApp.packageManager.getActivityIcon(activityName)
            }
        } else {
            try {
                fuqpApp.appIconLoader.loadIcon(appInfo).toDrawable(fuqpApp.resources)
            } catch (e: Throwable) {
                ServiceClient.log(Log.ERROR, TAG, e.stackTraceToString())

                try {
                    appInfo.loadIcon(fuqpApp.packageManager)
                } catch (x: Throwable) {
